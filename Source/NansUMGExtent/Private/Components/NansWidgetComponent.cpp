...
void UNansWidgetComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UNansWidgetComponent::ManageTouchEvents()
{
	...
	// No interaction here!
	if (GetWidget()->GetVisibility() != ESlateVisibility::SelfHitTestInvisible
		&& GetWidget()->GetVisibility() != ESlateVisibility::Visible)
	{
		return;
	}

	if (!InWidgetZone(TouchPos, GetWidget()->GetRootWidget())) return;
...
bool UNansWidgetComponent::InWidgetZone(FVector2D UserTouchPos, UWidget* WidgetToCheck)
{
	...
	UNansWidgetComponent::ForWidgetAndChildren(
		GetWidget(),
		[&AllWidgets](UWidget* ChildWidget)
		{
